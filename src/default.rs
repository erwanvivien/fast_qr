...
pub fn create_matrix<const N: usize>(version: Version) -> Matrix<N> {
...
#[cfg(test)]
mod test {
    use crate::default::create_matrix;
    use crate::module::Matrix;
    use crate::Version;

    /// Tests that the matrix is correctly created
    #[test]
    fn create_matrix_01() {
        const MAT: [[bool; 21]; 21] = [
            [
        ...
        let mat = create_matrix(Version::V01);
        let mat: [[bool; 21]; 21] = mat.map(|x| x.map(|x| x.into()));
    ...
    fn create_matrix_02() {
        ...
        let mat = create_matrix(Version::V02);
        let mat: [[bool; 25]; 25] = mat.map(|x| x.map(|x| x.into()));
    ...
    fn create_matrix_07() {
        ...
        let mat = create_matrix(Version::V07);
        let mat: [[bool; 45]; 45] = mat.map(|x| x.map(|x| x.into()));
