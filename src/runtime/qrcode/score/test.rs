...
fn score_total_vahan_dev() {
    assert_eq!(
        crate::runtime::qrcode::score::matrix_score(&MAT_VAHAN_DEV),
        441
    );
}
...
fn score_total_url_doesnt_exist() {
    assert_eq!(
        crate::runtime::qrcode::score::matrix_score(&MAT_URL_DOESNT_EXIST),
        586
    );
}
...
#[test]
fn score_total_vtf() {
    assert_eq!(crate::runtime::qrcode::score::matrix_score(&MAT_VTF), 572);
}
...
fn score_total_very_long_test() {
    assert_eq!(
        crate::runtime::qrcode::score::matrix_score(&MAT_VERY_LONG_TEST),
        567
    );
}
...
fn score_total_longer_test() {
    assert_eq!(
        crate::runtime::qrcode::score::matrix_score(&MAT_LONGER_TEST),
        617
    );
}
...
#[test]
fn score_total_a() {
    assert_eq!(crate::runtime::qrcode::score::matrix_score(&MAT_A), 343);
}

#[test]
fn score_line_only_false() {
    let tmp = [false; 60];
    assert_eq!(crate::runtime::qrcode::score::test_score_line(&tmp), 58);
}
...
fn score_line_only_false_one_true() {
    ...
    tmp[30] = true;
    assert_eq!(crate::runtime::qrcode::score::test_score_line(&tmp), 55);
}
...
fn score_line_12_first() {
    ...
    tmp[0] = true;
    assert_eq!(crate::runtime::qrcode::score::test_score_line(&tmp), 9);
}
...
fn score_line_12_second() {
    let mut tmp = [false; 12];
    tmp[1] = true;
    assert_eq!(crate::runtime::qrcode::score::test_score_line(&tmp), 8);
}
...
fn hello_world_mask0() {
    use crate::runtime::qrcode::score::{
